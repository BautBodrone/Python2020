...
def buscar_palabra(atril, dificultad, categoria):
    ...
    for i in range(2, 8):
        ...
        for each in it.permutations(atril,
                                    largo):  # se recorren un lista de permutacion que se obtiene con permutations
            palabra = convertirTupla(each)
            if correccion.palabraValida(palabra, dificultad, categoria):  # se pregunta si alguna palabra
                # corresponde con
