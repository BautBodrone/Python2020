def ventana_juego():
    ...
    def desbloquear_boton():
        """desbloquea todos los cuadrantes de la grilla"""
        for lista in matriz:
            for boton in lista:
                if boton.GetText() == "":
    ...
    def bloquear_boton():
        """bloquea todos los cuadrantes de la grilla"""
        for lista in matriz:
            for boton in lista:
    ...
    def generar_matriz(N=tamanio_matriz):
    ...
    def crear_atril(nombre):
        """crea 7 botones sin valor que se van a utilizar como atril en el juego"""
        atril_l = []
        for x in range(1, 8):
    ...
    def fin_juego(puntaje_total, puntaje_maquina, atril_jugador, atril_maquina, valores):
        ...
        if puntaje_total > puntaje_maquina:
            sg.Popup(
                "GANASTE!!! con: " + str(puntaje_total) + " puntos contra: " + str(puntaje_maquina) + " de la maquina")
            ...
            sg.Popup(
                "Gano la maquina con: " + str(puntaje_maquina) + " puntos contra: " + str(puntaje_total) + " tuyos")
    ...
    actual = ""  # es el el evento acutual
    event_anterior = ""  # es el evento anterior sirve para no pisar botones del atril
    atril_jugador = ("letra1", "letra2", "letra3", "letra4", "letra5", "letra6", "letra7")
    comenzar = False

    #  puede que tengamos que cambiarlo de lugar
    cambios_restantes = 3  # canti. de cambios de letras que le quedan al jugar
    ...
    while True:
        try:
            ...
            elif event in atril_jugador:  # entra si se preciona una letra del atril
                print("Tipo: ", event)
